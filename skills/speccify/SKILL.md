---
name: speccify
description: Bring a skill from a Speccify library into this project and use it
  in three phases — expand (adapt it here, implement its tools for this
  machine), execute, evaluate (try to prove it went wrong; iterate if it did).
  Use when a task is recurring and no installed skill covers it, when a skill
  under .agent/skills/ names a tool that has no implementation yet, or when
  the user mentions speccify, expand, tool spec, or .agent/skills.
license: MIT
compatibility: Requires the `speccify` CLI on PATH and a project with speccify.yaml
metadata:
  speccify.version: 0.1.0
