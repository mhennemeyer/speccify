...
**Phase 1d aktiv — Browser-Playground (`apps/web/`). Steps 0–4 abgeschlossen: FastAPI-Backend `speccify-web-backend` mit `/api/v1/specs` + `/api/v1/render` (offline gegen Replay-Cache), Next.js 15 / React 19 / TS-Frontend (pnpm) mit Spec-Picker, Monaco-YAML-Editor, Render-Output (TSX + `generator_pin`) und Error-Panel (`cache_miss`-Hint), **Cross-Consistency-Test CLI ↔ MCP ↔ Web byte-identisch grün** (`apps/web/backend/tests/test_cross_consistency.py`), `apps/web/README.md` + Top-Level-`README.md` aktualisiert. **183 Pytest grün**, Frontend `pnpm build` grün. Nächster Schritt: Step 5 (CI-Jobs `apps/web backend (offline)` + `apps/web frontend build`); danach Step 6 (Wrap-up, Tag-Vorschlag `v0.4.0-phase-1d`).**

Aktiver Plan: [`.agent/plans/phase-1d-browser-playground.md`](./.agent/plans/phase-1d-browser-playground.md) (Steps 0–4 abgeschlossen, Step 5 = CI-Jobs ist der nächste Schritt). Archivierte Phasen-Pläne: [`archive/phase-1c-mcp-server.md`](./.agent/plans/archive/phase-1c-mcp-server.md) (Tag-Vorschlag `v0.3.0-phase-1c`), [`archive/phase-1b-react-codegen.md`](./.agent/plans/archive/phase-1b-react-codegen.md) (Tag `v0.2.0-phase-1b`), [`archive/phase-1a-resolver-lockfile.md`](./.agent/plans/archive/phase-1a-resolver-lockfile.md) (Tag `v0.1.0-phase-1a`).
