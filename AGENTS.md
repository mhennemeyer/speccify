...
**Phase 1d aktiv — Browser-Playground (`apps/web/`). Steps 0–3 abgeschlossen: FastAPI-Backend `speccify-web-backend` mit `/api/v1/specs` + `/api/v1/render` (offline gegen Replay-Cache, byte-identisch zu CLI/MCP), Next.js 15 / React 19 / TS-Frontend (pnpm) mit Spec-Picker, Monaco-YAML-Editor, Render-Output (TSX + `generator_pin`) und Error-Panel (`cache_miss`-Hint). Backend 182 Pytest grün, Frontend `pnpm build` grün. Nächster Schritt: Step 4 — Cross-Consistency-Test CLI ↔ MCP ↔ Web (byte-identisch) + Top-Level-Doku-Sync; danach Step 5 (CI-Jobs) und Step 6 (Wrap-up, Tag-Vorschlag `v0.4.0-phase-1d`).**

Aktiver Plan: [`.agent/plans/phase-1d-browser-playground.md`](./.agent/plans/phase-1d-browser-playground.md) (Steps 0–3 abgeschlossen, Step 4 = Cross-Consistency-Test CLI ↔ MCP ↔ Web + Doku-Sync ist der nächste Schritt). Archivierte Phasen-Pläne: [`archive/phase-1c-mcp-server.md`](./.agent/plans/archive/phase-1c-mcp-server.md) (Tag-Vorschlag `v0.3.0-phase-1c`), [`archive/phase-1b-react-codegen.md`](./.agent/plans/archive/phase-1b-react-codegen.md) (Tag `v0.2.0-phase-1b`), [`archive/phase-1a-resolver-lockfile.md`](./.agent/plans/archive/phase-1a-resolver-lockfile.md) (Tag `v0.1.0-phase-1a`).
