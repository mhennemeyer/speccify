...
**Phase 1d abgeschlossen — Browser-Playground (`apps/web/`).** FastAPI-Backend `speccify-web-backend` mit `/api/v1/specs` + `/api/v1/render` (offline gegen Replay-Cache), Next.js 15 / React 19 / TS-Frontend (pnpm) mit Spec-Picker, Monaco-YAML-Editor, Render-Output (TSX + `generator_pin`) und Error-Panel (`cache_miss`-Hint). **Cross-Consistency-Test CLI ↔ MCP ↔ Web byte-identisch grün** (`apps/web/backend/tests/test_cross_consistency.py`). **CI** um zwei Jobs erweitert (`apps/web backend (offline)` Pytest + `apps/web frontend build` pnpm `--frozen-lockfile`). **183 Pytest grün**, Frontend `pnpm build` grün. Master-Plan `speccify-plan.md` synchronisiert (Phase 1d abgeschlossen, Tool-Vertrag `/api/v1/...` inline). **Tag-Vorschlag an User: `v0.4.0-phase-1d`** (selbst nicht gesetzt, vgl. `rules.md`). **Nächster Schritt: Phase 2 (Registry-MVP) — Plan-Entwurf nach User-Tag.**

Aktiver Plan: keiner — Phase 1d-Plan archiviert; Tag-Vorschlag `v0.4.0-phase-1d` weiterhin offen an User (vgl. `.agent/rules.md`). Archivierte Phasen-Pläne: [`archive/phase-1d-browser-playground.md`](./.agent/plans/archive/phase-1d-browser-playground.md) (Tag-Vorschlag `v0.4.0-phase-1d`), [`archive/phase-1c-mcp-server.md`](./.agent/plans/archive/phase-1c-mcp-server.md) (Tag-Vorschlag `v0.3.0-phase-1c`), [`archive/phase-1b-react-codegen.md`](./.agent/plans/archive/phase-1b-react-codegen.md) (Tag `v0.2.0-phase-1b`), [`archive/phase-1a-resolver-lockfile.md`](./.agent/plans/archive/phase-1a-resolver-lockfile.md) (Tag `v0.1.0-phase-1a`).
