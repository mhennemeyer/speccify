...
**Phase 5b Stage 4 abgeschlossen (2026-05-29) — voller 75-Pfad-Cross-Consistency-Sweep produktiv.** Neue zentrale Test-Datei `registry/tests/test_cross_consistency_sweep.py` parametrisiert die Matrix `5 Specs × 3 Targets = 15 Zellen` über alle 5 Bezugsweg-Pfade (Local/Remote/CLI/MCP/Web) → **75 byte-Vergleichsoperationen** pro Sweep-Lauf. Pfad-APIs: Local = `LocalRegistry.fetch` + `render_for_target`; Remote = Publish über `live_server` + `RemoteRegistry`; CLI = `cli.commands.lock.run_lock` + `cli.commands.pull.run_pull` gegen dynamisch erzeugtes Mini-Manifest; MCP = `mcp.tools.pull.run_pull` analog (Function-Call-Level, OQ-Decision); Web = `render_spec_from_yaml`. Web-Pfad ist heute target-agnostisch (OQ5=A bestätigt — kein `xfail` nötig). Bestehende Per-Pfad-Tests (`test_remote_multi_target.py`, `test_cross_consistency.py`) bleiben unverändert. **Verifikation: 332 Root-Pytest Default + 134 Registry-Pytest (+15 ggü. Stage 3) = 466 Tests gesamt grün; 15 Sweep-Parametrisierungen passed in 2.77 s lokal; ruff/format clean.** Plan `phase-5b-conformance-sweep.md` aktualisiert: Stages 0–4 Done, Stages 5–6 offen. **Nächster Schritt: Stage 5 (CI-Update + Docs-Polish) → Stage 6 (Plan-Archivierung + Tag-Vorschlag `v0.9.0-phase-5b`).**

**Phase 5b Stages 1–3 abgeschlossen (2026-05-29) — echte Cross-Spec×Target-Conformance produktiv für Angular und SwiftUI.** Stage 1 (2026-05-27) hat `scripts/record_llm_cache.py` target-aware gemacht (`--target react|angular|swiftui|all|<csv>`); Stage 2 (2026-05-29) wurde durch den User abgeschlossen — Replay-Cache-Fixtures für `5 Specs × {angular, swiftui}` unter `tests/fixtures/llm-cache/` committed (12 neue JSONs); Stage 3 (2026-05-29) ersetzt die synthetischen Mini-Snippets in `core/tests/test_conformance_build_smoke.py` durch echte parametrisierte Tests über alle 5 Phase-0-Referenz-Specs — neue Helfer `_render_spec(spec_id, version, target)` + `_lock_entry_for(...)` als target-agnostische Faktorisierung, React-Test bleibt unverändert. **Verifikation: 332 Root-Pytest Default (`-m "not conformance"`, unveränderter Count — neue Tests sind `@conformance`-markiert) + 11 Conformance-Tests passed in 8.66 s lokal (1 React + 5 Angular + 5 SwiftUI parametrisiert); ruff/format clean.**
