...
> **Update 2026-05-06 (Domain-Status)**: `speccify.io` + `speccify.de` durch Owner registriert (Anbieter df.eu). `speccify.dev` ist bei df.eu **nicht verfügbar/anbietbar**; defensives Halten von `.dev` damit aufgeschoben — optional später über Spezialanbieter (Cloudflare Registrar, Namecheap, Squarespace), falls aktiv benötigt.
