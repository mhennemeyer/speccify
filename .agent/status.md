...
## Meta
- **Typ:** Code
- **Phase:** Phase 1b **abgeschlossen** und getaggt (`v0.1.0-phase-1a` → `d28cb33`, `v0.2.0-phase-1b` → `8c90511`, lokal annotated; kein Git-Remote konfiguriert, also kein Push). **Phase 1c läuft**: Phasen-Plan `.agent/plans/phase-1c-mcp-server.md` aktiv. **Steps 0–4 abgeschlossen** (2026-05-16): SDK `mcp[cli]>=1.27.1,<2.0` gepinnt; Server-Skeleton + Read-only Tools `resolve`/`lint`/`render` + Write-Tools `lock`/`pull`/`verify` + Resources `speccify://manifest|lockfile` und `spec://{scope}/{name}@{version}` + Prompt `add-spec` registriert (dünne Adapter über `speccify_core`). Cross-Consistency CLI ↔ MCP byte-identisch. **173 Tests grün**, ruff/format clean. Nächster Schritt: Step 5 (CI-Smoke `speccify-mcp` stdio offline + README/`mcp/README.md`).
- **Priorität:** Mittel
