...
## Meta
- **Typ:** Code
- **Phase:** **Phase 1d abgeschlossen + archiviert** (2026-05-21). Phasen-Plan liegt jetzt unter [`.agent/plans/archive/phase-1d-browser-playground.md`](./plans/archive/phase-1d-browser-playground.md) (`isActive: false`). Getaggt (lokal annotated, kein Git-Remote): `v0.1.0-phase-1a` → `d28cb33`, `v0.2.0-phase-1b` → `8c90511`. Tag-Vorschläge `v0.3.0-phase-1c` und `v0.4.0-phase-1d` weiterhin an User offen (selbst nicht gesetzt, vgl. `rules.md`). Phase 1d umfasste den Browser-Playground unter `apps/web/`: FastAPI-Backend `speccify-web-backend` (`/api/v1/specs`, `/api/v1/render`, offline gegen Replay-Cache) + Next.js 15 / React 19 / TS-Frontend (pnpm@10.33.3, Node ≥22) mit Spec-Picker, Monaco-YAML-Editor, Render-Output (TSX + `generator_pin`) und Error-Panel; Cross-Consistency CLI ↔ MCP ↔ Web byte-identisch (`apps/web/backend/tests/test_cross_consistency.py`); zwei neue CI-Jobs `apps/web backend (offline)` + `apps/web frontend build`. **183 Tests grün**, ruff/format clean, Frontend `pnpm build` grün. Nächster Schritt: Phase-2-Plan-Entwurf (Registry-MVP), nachdem User den Tag `v0.4.0-phase-1d` gesetzt hat.
- **Priorität:** Mittel
