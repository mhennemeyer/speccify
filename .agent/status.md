...
## Nächste Schritte
- **AKTIV: Toolkit-Vollausbau** — Plan [`plans/toolkit-discovery-terminal.md`](./plans/toolkit-discovery-terminal.md) (Entwurf 2026-07-25, **wartet auf BO-Refinement der 8 Open Questions**). Ziele: Discovery-MCP (MCPs+Tools mit Client-Config), Toolbox wie dotagent (TOML-Manifeste), MCPs Exec/Parallels/Playwright, eigene Tools/MCPs im Working Dir (Settings), Terminal-Seitenleiste mit Claude Code im Working Dir (Einweisung per Klick anlegbar), ask_bo-Chat-Elemente (buttons/multi_select/form, iKanbanAi-Schema). Vorgeschlagene Reihenfolge: Settings → Toolbox → **Discovery (iKanbanAi wartet!)** → Exec-Rust-Port → Parallels/Playwright → Terminal → ask_bo. Danach Composer/Specs-Schärfung wieder aufnehmen (P3-Rest, P4, P5).
- Desktop-App: A0+A1 geliefert und vom BO abgenommen ([`plans/desktop-app-und-composer.md`](./plans/desktop-app-und-composer.md) abgeschlossen; Walkthrough für BO-Test: [`docs/composer-tristate-walkthrough.md`](../docs/composer-tristate-walkthrough.md), FilePicker drin). Rust-Fundament R0 ✅ ([`plans/rust-neustart-toolkit-mcps.md`](./plans/rust-neustart-toolkit-mcps.md) = Kontrakt-Referenz).
