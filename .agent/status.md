...
## Meta
- **Typ:** Code
- **Phase:** **Phase 3 aktiv — Skelett-Plan** (2026-05-24). Plan: [`.agent/plans/phase-3-codegen-targets.md`](./plans/phase-3-codegen-targets.md) (`isActive: true`); Stage 0 = Klärung von 10 Open Questions (zweites/drittes Target = SwiftUI/Angular/Compose, Conformance-Runner-Stack, Workspaces-Lockfile-Layout, Multi-Target-Manifest, Codegen-Modus pro Target, Golden-Renders, Visual-Refs, Workspaces-Resolver-Semantik, MCP-Tool-Vertrag). Phase 2 abgeschlossen + archiviert (2026-05-24). Phasen-Plan liegt unter [`.agent/plans/archive/phase-2-registry-mvp.md`](./plans/archive/phase-2-registry-mvp.md) (`isActive: false`). Getaggt (lokal annotated, kein Git-Remote): `v0.1.0-phase-1a` → `d28cb33`, `v0.2.0-phase-1b` → `8c90511`. Tag-Vorschläge `v0.3.0-phase-1c`, `v0.4.0-phase-1d` und **`v0.5.0-phase-2`** weiterhin an User offen (selbst nicht gesetzt, vgl. `rules.md`). Phase 2 lieferte den **Registry-MVP** unter `registry/`: Django-5-Backend mit REST-API `/api/v1/registry/...` (Publish/Fetch/Versions/Search/Yank/Whoami/Tokens/Device-Code), Django-Templates-Web-UI (Auth + Browse), Bearer-Tokens + TOTP-2FA + Device-Code-Login, Lockfile-Schema v2 (`yank_status` + `signature`-Slot, v1-kompat), Resolver-Multi-Registry mit `RemoteRegistry` (httpx + File-Cache + Server-Hash-Verify + `ScopeRegistryConflictError` für Dependency-Confusion-Schutz), CLI `login`/`whoami`/`publish`/`yank` + MCP-Tools `publish`/`yank` (Tool-Count 6→8). Cross-Consistency erweitert auf den Registry-Pfad (`registry/tests/test_cross_consistency_registry.py`); neuer CI-Job `registry backend (django + postgres)` mit Postgres-16-Service-Container. **Verifikation: 220 Root-Pytest grün, 116 Registry-Pytest grün → 336 Tests gesamt**, ruff/format clean. Nächster Schritt: **Phase-3-Plan-Entwurf** (zweites + drittes Codegen-Target, Conformance-Runner, Workspaces) nach User-Tag.
- **Priorität:** Mittel
...
- [x] **Phase-2-Plan-Skelett** geschrieben (2026-05-22): [`.agent/plans/phase-2-registry-mvp.md`](./plans/phase-2-registry-mvp.md) (`isActive: true`). Scope (Django-Backend `registry/`, Auth+2FA, `publish`/`search`/`yank`/`login`/`whoami`, Web-UI, Lockfile-Bump auf `schema_version: 2` mit `signature`+`yank_status`, RemoteRegistry-Resolver-Integration, Workspaces nativ), 10 Open Questions als Round-1-Klärung mit User, Stages 0–9 als Skelett (Delivery-Steps folgen in Round 2 nach Klärung). Keine Code-Änderungen in dieser Session — nur Plan-Hygiene analog zu früheren Phasen-Kickoffs.
- [x] **Phase-2 Stage 0 abgeschlossen** (2026-05-23): Alle 10 Open Questions mit User geklärt (Django 5.x, Django-Templates in `registry/`, Postgres BYTEA, TOTP only, opaque Tokens, Device-Code-Login, Self-Service-Scopes mit Sperrliste, Workspaces auf Phase 3 verschoben, Web-UI in `registry/`, kein Default-Registry-Hostname). Workspaces-Stage aus Phase-2-Plan gestrichen, Round-2-Delivery-Steps (Step 1–8) in [`phase-2-registry-mvp.md`](./plans/phase-2-registry-mvp.md) eingefügt.
