...
## Meta
- **Typ:** Code
- **Phase:** **Phase 1d abgeschlossen** (2026-05-19). Getaggt (lokal annotated, kein Git-Remote): `v0.1.0-phase-1a` → `d28cb33`, `v0.2.0-phase-1b` → `8c90511`. Tag-Vorschläge `v0.3.0-phase-1c` und `v0.4.0-phase-1d` an User offen (selbst nicht gesetzt, vgl. `rules.md`). Phase 1d umfasste den Browser-Playground unter `apps/web/`: FastAPI-Backend `speccify-web-backend` (`/api/v1/specs`, `/api/v1/render`, offline gegen Replay-Cache) + Next.js 15 / React 19 / TS-Frontend (pnpm@10.33.3, Node ≥22) mit Spec-Picker, Monaco-YAML-Editor, Render-Output (TSX + `generator_pin`) und Error-Panel; Cross-Consistency CLI ↔ MCP ↔ Web byte-identisch (`apps/web/backend/tests/test_cross_consistency.py`); zwei neue CI-Jobs `apps/web backend (offline)` + `apps/web frontend build`. **183 Tests grün**, ruff/format clean, Frontend `pnpm build` grün. **Step 6 abgeschlossen**: Master-Plan `speccify-plan.md` Phase 1d inline mit Tool-Vertrag `/api/v1/...`-Endpoints + Fehler-Codes + Offline-/Replay-Cache-Limitierung dokumentiert; `AGENTS.md` „Aktuelle Phase" auf Phase 1d abgeschlossen / nächste Phase 2 (Registry-MVP) umgestellt. Nächster Schritt: Phase-2-Plan-Entwurf (Registry-MVP), nachdem User den Tag `v0.4.0-phase-1d` gesetzt hat.
- **Priorität:** Mittel
...
- Nächster Schritt: **Phase-2-Plan-Entwurf (Registry-MVP)** nach User-Tag `v0.4.0-phase-1d`; danach Phase-1d-Plan archivieren analog zu 1a/1b/1c.
