...
## Nächste Schritte
- **Workstream Desktop/Infrastruktur (BO-Umpriorisierung 2026-07-24):** Plan [`plans/desktop-app-und-composer.md`](./plans/desktop-app-und-composer.md) — **A0 ✅** (dotagent-App → `apps/desktop`, Rebranding „Speccify"/`io.speccify.desktop`, CLI-Bridge zeigt weiter auf dotagent-CLI) und **A1 ✅** (Composer-Fenster: Backend serviert die SPA same-origin unter `/ui`, Rust-Command `open_composer` spawnt `speccify-web-backend` auf freiem Port via Supervisor, Fenster-Close killt den Prozess; „Composer" ist Default-Tab). Danach **A2 Feinschliff nach BO-Rumprobieren**, dann **Migration nach Rust** [`plans/rust-neustart-toolkit-mcps.md`](./plans/rust-neustart-toolkit-mcps.md) — **R0 dort ✅** (Cargo-Workspace `crates/{exec-mcp,discovery-mcp}`, `schema/actions.schema.json`, Wire-Vertrag `docs/exec-mcp-contract.md` + Diff-Harness, 28/28 Parität gegen dotagent-Referenz); R1 (Rust-Exec-MCP) startet nach BO-Go.
- **BO-Check offen:** `pnpm run desktop:dev` → App „Speccify" mit 5 Tabs; im Composer-Tab „Composer-Fenster öffnen" (braucht einmal `uv sync` + `pnpm run composer:build` im Repo). Fertige Bundles: `target/release/bundle/macos/Speccify.app` + `…/dmg/Speccify_0.1.0_aarch64.dmg`.
