"""Phase-2 Lockfile-v2-Tests: yank_status, signature-Slot, v1→v2-Migration."""
...
def test_lockfile_default_schema_version_is_v2() -> None:
    lock = Lockfile(target="react")
    assert lock.schema_version == 2
    assert CURRENT_LOCKFILE_SCHEMA_VERSION == 2
...
def test_lockfile_v2_round_trip_with_yank_and_signature(tmp_path: Path) -> None:
    lock = Lockfile(
        target="react",
        entries=(
...
def test_lockfile_v1_loads_and_migrates_to_v2(tmp_path: Path) -> None:
    """Phase-1-Lockfiles (schema_version: 1, ohne signature/yank_status) bleiben lesbar."""
    ...
    # Beim Re-Write wird das Lockfile auf v2 aktualisiert.
...
def test_lockfile_v2_rejects_invalid_yank_status(tmp_path: Path) -> None:
