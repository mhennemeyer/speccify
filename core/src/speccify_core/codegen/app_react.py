...
def _package_json(spec: Spec) -> bytes:
    return _json_file(
        {
            "name": _project_name(spec.name_id),
...
def _index_html(spec: Spec) -> bytes:
    title = str(spec.parsed().get("title", spec.name_id))
    return _text(
        [
            "<!doctype html>",
            '<html lang="de">',
...
def _main_tsx(spec: Spec) -> bytes:
    return _text(
        [
            ...
            f"// Spec: {spec.name_id}@{spec.version}",
...
def _readme(spec: Spec, app: AppSpec, *, mocks: bool) -> bytes:
    parsed = spec.parsed()
    lines = [
        f"# {parsed.get('title', spec.name_id)}",
        "",
        f"Generiert aus `{spec.name_id}@{spec.version}` mit `speccify build`",
...
def _component_barrel(child: Spec) -> bytes:
    component = _component_name(child.name_id)
    return _text(
        [
            f"// AUTO-GENERATED by `speccify build` — {APP_TEMPLATE_SET} v{APP_TEMPLATE_VERSION}",
            "//",
...
def _screen_jsx(
    ...
    *,
    ...
    component = _component_name(child_spec.name_id)
...
def _app_tsx(
    ...
    for alias in composition.uses:
        ...
        if component in seen:
            continue
        seen.add(component)
        path = _component_import(child.name_id)
    ...
    for rule in composition.wiring:
        ...
        if rule.emit is not None:
            ...
            lines.append(
                f'      console.debug("[speccify] {spec.name_id} emittiert '
...
def render_app_project(
    ...
    *,
    ...
    if str(parsed.get("kind", "")) != "app":
        raise AppCodegenError(
            f"{spec.name_id}: `speccify build` erwartet `kind: app`, "
            f"nicht '{parsed.get('kind', '')}'."
        )
    app = parse_app(parsed)
    if app is None:
        raise AppCodegenError(f"{spec.name_id}: `kind: app` ohne `app:`-Block ist nicht baubar.")
    composition = parse_composition(parsed)
    if composition is None:
        raise AppCodegenError(f"{spec.name_id}: App ohne `composition:` hat keine Screens.")
    ...
    if missing:
        raise AppCodegenError(f"{spec.name_id}: Routen zeigen auf unbekannte Knoten {missing}.")
    ...
    if mocks:
        for child in children.values():
            ...
            files[_component_path(child.name_id)] = _component_barrel(child)
    else:
        ...
        for rel_path, data in components.items():
            files[f"{COMPONENT_DIR}/{rel_path}"] = data
        for child in children.values():
            expected = _component_path(child.name_id)
            if expected not in files:
                raise AppCodegenError(
                    f"Implementierung für {child.name_id} fehlt (erwartet '{expected}')."
                )
