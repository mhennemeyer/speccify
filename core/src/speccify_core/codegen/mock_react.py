...
def _header_lines(spec: Spec) -> list[str]:
    return [
        ...
        f"// Spec: {spec.name_id}@{spec.version} · {MOCK_TEMPLATE_SET} v{MOCK_TEMPLATE_VERSION}",
...
def _leaf_component_lines(spec: Spec, api: ComponentApi) -> list[str]:
    component = _component_name(spec.name_id)
    lines: list[str] = []
    lines.append(
        f"export default function {component}(props: {component}Props): React.ReactElement {{"
    )
    lines.append("  return (")
    lines.append(f'    <div data-speccify-mock="{spec.name_id}" style={{mockStyles.container}}>')
...
def _composite_node_jsx(
    ...
    *,
    ...
    child_component = _component_name(child_spec.name_id)
...
def _logic_mock_source(spec: Spec, api: ComponentApi) -> str:
    if not api.fixtures:
        raise MockUnavailableError(
            f"Spec {spec.name_id}@{spec.version} (kind: logic) deklariert keine "
...
def render_mock_files(spec: Spec, children: Mapping[str, Spec] | None = None) -> dict[str, bytes]:
    ...
    if kind == "logic":
        ...
        return {_output_path(spec.name_id, kind=kind): source.encode("utf-8")}
    ...
    if composition is not None:
        ...
        if missing:
            raise MockCodegenError(
                f"Composite {spec.name_id}: Kind-Specs für Aliase {missing} nicht übergeben."
            )
        seen_imports: set[str] = set()
        for alias in composition.uses:
            child = resolved_children[alias]
            child_component = _component_name(child.name_id)
            if child_component in seen_imports:
                continue
            seen_imports.add(child_component)
            path = _import_path(spec.name_id, child.name_id)
...
def render_mock_closure(spec: Spec, registry: Registry) -> MockRender:
    ...
    def _render(current: Spec) -> None:
        marker = f"{current.name_id}@{current.version}"
