...
class GitRegistry:
    ...
    def _run_bytes(self, args: list[str], *, cwd: Path | None = None) -> bytes:
        """Ruft git auf und liefert rohe stdout-Bytes (Spec-Hashes müssen exakt bleiben)."""
    ...
    def _run(self, args: list[str], *, cwd: Path | None = None) -> str:
        return self._run_bytes(args, cwd=cwd).decode("utf-8", "replace")

    def _repo_dir(self, ref: GitRef) -> Path:
        digest = hashlib.sha256(ref.url.encode("utf-8")).hexdigest()[:16]
        # Lesbarer Präfix + Hash: im Cache-Verzeichnis erkennt man das Repo wieder,
        # der Hash hält die Zuordnung eindeutig.
        name = ref.url.rstrip("/").rsplit("/", 1)[-1].removesuffix(".git") or "repo"
        return self._cache_dir / f"{name}-{digest}" / "repo.git"

    def _ensure_repo(self, ref: GitRef, *, fetch: bool) -> Path:
        repo = self._repo_dir(ref)
        if not repo.is_dir():
            if self._offline:
                raise GitRegistryError(
                    f"{ref.spec_id}: kein Cache unter {repo} und offline=True — "
                    f"einmal online auflösen (z. B. `speccify lock`) oder Cache mitliefern."
                )
            repo.parent.mkdir(parents=True, exist_ok=True)
            self._run(["init", "--bare", "--quiet", str(repo)])
            self._run(["remote", "add", "origin", ref.url], cwd=repo)
        if fetch and not self._offline and ref.url not in self._fetched:
            self._run(
                ["fetch", "--quiet", "--depth", "1", "--tags", "--force", "origin"],
                cwd=repo,
            )
    ...
    def list_versions(self, spec_id: str) -> list[Version]:
        ...
        repo = self._ensure_repo(ref, fetch=True)
    ...
    def fetch(self, spec_id: str, version: Version) -> Spec:
        ...
        try:
            blob = self._run_bytes(["cat-file", "blob", f"{tag}:{ref.spec_path}"], cwd=repo)
        except GitRegistryError as exc:
            raise GitRegistryError(
