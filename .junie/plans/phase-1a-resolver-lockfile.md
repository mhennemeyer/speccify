...
Phase 0 ist abgeschlossen (Schema, `flowcation lint`, 5 Specs, CI). Nächster Schritt ist **Phase 1a — Resolver + Lockfile + `add`/`pull` mit Stub-Codegen**. Bewusster Sub-Spike, **nicht** das ganze Phase-1-Paket: SwiftUI/React-Codegen-Vollausbau, MCP-Server und Browser-Playground bleiben in 1b/1c/1d.

Ziel: Ein Toy-Projekt im Repo (`example-project/`) kann via
```
flowcation add @org/button
flowcation pull --target react --out ./out
flowcation verify
```
...
- Projekt-Manifest `flowcation.yaml` (anders als Spec-YAML): `target`, `dependencies`, `registry`-Pfad.
- Lokale Pseudo-Registry: `registry-fixtures/<scope>/<name>/<version>/spec.flowcation.yaml` (per `--registry` / `flowcation.toml` konfigurierbar).
- **Resolver in `flowcation_core`**: MVS strikt (Go-Vorbild), transitiv über `uses:`, mit Diamond-Auflösung.
- **Lockfile `flowcation.lock`**: Spec-Bundle-Hashes, `generator{kind: template, template_set, template_version}`, Output-Hashes pro Datei. Eigenes JSON-Schema unter `schema/lockfile.schema.json`.
- **CLI-Befehle**: `flowcation add <id>`, `flowcation lock`, `flowcation pull --target react`, `flowcation verify`. `flowcation init` und `flowcation publish`/`yank`/`search` bleiben Phase 1b/2.
- **Stub-Codegen**: Markdown-Renderer (`<id>.md`) mit deterministischen Jinja-artigen Templates aus `flowcation_core.codegen`.
...
- `flowcation init` Boilerplate-Generator (Phase 1b — folgt natürlich, wenn Codegen reifer ist).

### User Stories

- *Als Spec-Konsument* will ich `flowcation add @org/button` aufrufen, damit `flowcation.yaml` und `flowcation.lock` automatisch aktualisiert werden — ohne Spec-Inhalte manuell zu kopieren.
- *Als CI-Pipeline* will ich `flowcation verify` ausführen, damit ich sicher bin, dass weder die Spec-Bundles noch die generierten Outputs seit dem letzten Commit driften.
...
- `flowcation add @scope/name[@<range>]`: Schreibt Eintrag in `flowcation.yaml` (Default-Range `^<latest-major.minor>` oder exakt, falls `=` angegeben), löst auf und ruft intern `lock` auf.
- `flowcation lock`: Liest Manifest, resolved transitive Dependencies via MVS, schreibt `flowcation.lock` mit Spec-Hashes (kein Codegen-Aufruf).
- `flowcation pull --target react --out <dir>`: Liest Lockfile, lädt resolved Specs, ruft Stub-Codegen, schreibt Output, ergänzt Output-Hashes im Lockfile.
- `flowcation verify`: Re-resolved, re-generiert in temporäres Verzeichnis, vergleicht *beide* Hash-Sätze; exit 0 nur bei vollständiger Übereinstimmung.
...
- `cli/src/flowcation_cli/__main__.py` — Typer-App mit `lint`. Wird um neue Sub-Commands erweitert; bestehende `lint`-Logik bleibt unverändert.
- `schema/spec.schema.json` — bleibt unverändert; daneben kommen `schema/manifest.schema.json` und `schema/lockfile.schema.json`.
- `specs/*.flowcation.yaml` — werden in `registry-fixtures/<scope>/<name>/<version>/spec.flowcation.yaml` gespiegelt (oder symlinked) als seed für Tests.
...
3. **Lokales Pseudo-Registry.** Verzeichnis-Layout `registry-fixtures/<scope>/<name>/<version>/spec.flowcation.yaml`. Pfad konfigurierbar via `flowcation.toml` Key `registry.path`, überschreibbar via `--registry` Flag. Kein Netzwerk in Phase 1a.
...
Neues Modul `flowcation_core.manifest`:
...
Neues Modul `flowcation_core.registry`:
...
Neues Modul `flowcation_core.resolver`:
...
Neues Modul `flowcation_core.lockfile`:
...
Neues Modul `flowcation_core.codegen.stub`:
...
`cli/src/flowcation_cli/__main__.py` bekommt vier neue Sub-Commands über je ein eigenes Modul `commands/<name>.py`:
...
```yaml
# flowcation.yaml (Project Manifest)
schema_version: 1
target: react
registry:
...
- **`flowcation_core.manifest`** — neu; reine Datei-I/O + Schema-Validation für `flowcation.yaml`.
- **`flowcation_core.registry`** — neu; abstrakte `Registry`-Basis + konkretes `LocalRegistry`.
- **`flowcation_core.resolver`** — neu; MVS-Implementierung, `ResolverError`-Hierarchie.
- **`flowcation_core.lockfile`** — neu; Lockfile-Read/Write mit Schema-Validation.
- **`flowcation_core.codegen.stub`** — neu; Stub-Codegen-Modul; Template-Set ist statisch versioniert.
- **`flowcation_cli.commands`** — neu; jeweils dünner Adapter über die `core`-Module. `lint` bleibt unangetastet.
...
- **`example-project/`** — neu; minimales `flowcation.yaml` für Smoke-Test in CI.
...
registry-fixtures/       # neu
  org/button/0.1.0/spec.flowcation.yaml
  org/button/0.1.1/spec.flowcation.yaml
  org/contact-form/0.1.0/spec.flowcation.yaml
  org/http-api-client/0.1.0/spec.flowcation.yaml
  org/onboarding-wizard/0.1.0/spec.flowcation.yaml
  org/login-screen/0.1.0/spec.flowcation.yaml
example-project/         # neu
  flowcation.yaml
.agent/plans/
...
```mermaid
graph TD
    M[flowcation.yaml<br/>Project Manifest] --> R[Resolver<br/>MVS, transitiv]
    REG[registry-fixtures/<br/>LocalRegistry] --> R
    R --> RG[ResolvedGraph]
    RG --> LW[Lockfile Writer]
    LW --> LF[flowcation.lock<br/>spec-hashes + generator-pin]
    LF --> CG[Stub-Codegen<br/>Jinja templates]
    CG --> OUT[./out/<br/>generated *.md]
    OUT --> H[file hashes]
    H --> LF
    LF --> V[flowcation verify<br/>re-resolve + re-render + diff]
```
...
- **`flowcation lint` und `flowcation.yaml`**: Bestehender `lint` würde das neue Manifest fälschlicherweise gegen das Spec-Schema validieren. Mitigation: `lint` wertet `*.flowcation.yaml`-Suffix oder `kind`-Präsenz aus und überspringt Manifeste — kleinster Patch in 1a.
...
- **Manifest-Round-Trip**: Lade `flowcation.yaml`, schreibe es zurück, Bytes sind identisch.
...
- **`flowcation add @org/button` E2E**: leeres `flowcation.yaml` + Pseudo-Registry → Manifest enthält `"@org/button": "^0.1"`, Lockfile hat einen Eintrag mit korrektem Hash.
- **`flowcation pull --target react --out ./out` E2E**: Output-Datei `out/org/button.md` existiert und enthält Title + Acceptance; Lockfile hat `generated_files_sha256` mit passendem Hash.
- **`flowcation verify` Happy-Path**: Nach `pull` läuft `verify` mit exit 0.
- **`flowcation verify` Drift-Detection**: Manuell veränderte Output-Datei → exit 1 mit klarem Diff-Hinweis.
...
- `pull` ohne `lock` → CLI-Fehler „run `flowcation lock` first".
- `verify` mit veraltetem `template_version` → exit 1, Hinweis auf nötigen `flowcation pull`.
...
- CI (`.github/workflows/ci.yml`): zusätzlicher Step `cd example-project && uv run flowcation lock && uv run flowcation pull --target react --out ./out && uv run flowcation verify`.
...
3. **Phase-1-Aufteilung in Sub-Spikes** (Master-Plan, Zeile 282–288).
   - Ergänzen: Phase 1 wird in Sub-Spikes 1a (Resolver/Lockfile/Stub-Codegen, dieses Dokument), 1b (echtes React-Codegen + `flowcation init`), 1c (MCP-Server), 1d (Browser-Playground) zerlegt.
...
`flowcation.yaml`-Projektmanifest und das lokale Pseudo-Registry können geladen, validiert und durchsucht werden.

- `schema/manifest.schema.json` neu anlegen (`schema_version`, `target`, `dependencies`, optional `registry.path`).
- `flowcation_core.manifest.ProjectManifest` als immutable Dataclass mit `load`/`write` (deterministische Key-Reihenfolge).
- `flowcation_core.registry.LocalRegistry` mit `list_versions(spec_id)` und `fetch(spec_id, version)` gegen Layout `registry-fixtures/<scope>/<name>/<version>/spec.flowcation.yaml`.
...
- `example-project/flowcation.yaml` als Minimal-Manifest für End-to-End-Smoke.
...
- `flowcation_core.resolver` neu: `Version`, `Range` (in 1a nur `^X.Y` und exakt; `~`/Pre-Releases ausgeklammert), `Resolution`, `ResolvedGraph`, `Resolver`, `ResolverError`-Hierarchie.
...
###   Step 3: Lockfile-Format, `flowcation lock` und `flowcation add`
...
- `flowcation_core.lockfile` mit `Lockfile.load`/`write` (deterministischer YAML-Dump, sortierte Keys).
- `flowcation_cli/commands/lock.py`: lädt Manifest, ruft Resolver, schreibt Lockfile *ohne* Codegen-Aufruf (Output-Hashes bleiben leer).
- `flowcation_cli/commands/add.py`: erwartet `<spec-id>[@<range>]`, fügt Eintrag in `flowcation.yaml` ein (Default-Range `^<major.minor>`), ruft intern `lock`.
...
###   Step 4: Stub-Codegen und `flowcation pull --target react`
`flowcation pull` rendert resolved Specs deterministisch nach Markdown und füllt Output-Hashes im Lockfile.

- `flowcation_core.codegen.stub` mit `TEMPLATE_SET="phase-1a-stub"`, `TEMPLATE_VERSION="0.1.0"`, Funktion `render(spec, target) -> dict[str, bytes]`.
- Jinja2-Template `templates/stub.md.j2`: Title, Summary, Inputs/Outputs/Events/Acceptance als Tabellen, `target` im Frontmatter.
- `flowcation_cli/commands/pull.py`: liest Lockfile, fordert resolved Specs aus `LocalRegistry`, ruft Stub-Codegen, schreibt Dateien nach `--out`, ergänzt `generated_files_sha256` im Lockfile (atomar via tempfile).
...
###   Step 5: `flowcation verify`, `lint`-Anpassung, CI-Step und Master-Plan-Sync
Verify schließt den Reproduzierbarkeits-Kreis und Phase 1a wird operativ wie dokumentarisch abgeschlossen.

- `flowcation_cli/commands/verify.py`: re-resolved Manifest, re-rendered in Temp-Verzeichnis, vergleicht *beide* Hash-Sätze (Spec-Bundle und Output-Dateien); exit 0 nur bei vollständiger Übereinstimmung; bei Drift exit 1 mit Liste betroffener Pfade.
- `flowcation_cli/__main__.py` `lint`-Logik anpassen: `flowcation.yaml` (kein `kind`-Feld) wird übersprungen oder gegen Manifest-Schema validiert, statt fälschlich gegen Spec-Schema.
- `.github/workflows/ci.yml` um Schritt erweitern: `cd example-project && uv run flowcation lock && uv run flowcation pull --target react --out ./out && uv run flowcation verify`.
