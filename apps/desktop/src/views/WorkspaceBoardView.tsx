...
export default function WorkspaceBoardView({ workspaceId, revision, refresh = 0, onSelect, listSlots }: {
  ...
  useEffect(() => {
    ...
    void invoke<Snapshot>("workspace_board", { workspaceId }).then(value => {
      if (!cancelled) {
        if (value.workspace_id !== workspaceId) { setError(t("Workspace-Antwort gehört zu einem anderen Kontext.")); return; }
        setSnapshot(value);
      }
  ...
  return <section aria-label="Workspace-Specs" className={`min-w-0 space-y-3 ${listSlots ? "flex h-full min-h-0 flex-col" : ""}`}>
    <div className="flex flex-wrap items-end gap-3">
      <label className="text-xs text-slate-500">Projekt
        <select aria-label={t("Board-Projekt")} value={filter} onChange={event => setFilter(event.target.value)} className="ml-2 max-w-full rounded border border-slate-300 bg-white p-2 text-slate-800">
          <option value="">{t("Alle Projekte")}</option>
          {projects.map(([id,name]) => <option key={id} value={id}>{name}</option>)}
          {filter && !projects.some(([id]) => id === filter) && <option value={filter}>{t("Projekt nicht mehr im Snapshot")}</option>}
        </select>
      </label>
      <input aria-label={t("Workspace-Specs suchen")} placeholder={t("Specs, Projekte, Repos suchen…")} value={search} onChange={event => setSearch(event.target.value)} className="min-w-40 flex-1 rounded border border-slate-300 bg-white px-3 py-2 text-sm text-slate-800" />
      <button className={button} disabled={loading} onClick={() => setReload(value => value + 1)}>{t("Board aktualisieren")}</button>
    </div>
    <p role="status" className="text-xs text-slate-500">{loading ? t("Snapshot laden…") : t("{visible} von {data} Specs", { visible: visible.length, data: data?.specs.length ?? 0 })}
      {data && t(" · Stand {captured_at}", { captured_at: new Date(data.captured_at).toLocaleTimeString() })}. {onSelect ? t("Bearbeitung im Inspektor betrifft das gekennzeichnete Projekt. Team-Sync erfolgt über die eingerichteten Register je Repository.") : t("Lesende lokale Übersicht. Änderungen nach Aktualisieren sichtbar.")}</p>
    {error && <ErrorBox message={`${error}${data ? t(" · Letzten Snapshot anzeigen, möglicherweise veraltet.") : ""}`} />}
    {data && data.revision !== revision && <p role="status" className="text-xs text-amber-700">{t("Projektzuordnung inzwischen geändert. Board und Workspace erneut aktualisieren.")}</p>}
    {data?.partial && <aside role="status" data-tone="amber" className="tone-surface rounded border p-3 text-xs"><strong>{t("Board unvollständig")}</strong><ul className="mt-1 list-inside list-disc break-all">{data.warnings.map((warning,index) => <li key={index}>{warning}</li>)}</ul></aside>}
    {!loading && !error && visible.length === 0 && <p className="text-sm text-slate-500">{t("Keine Specs für diese Auswahl. Projektfilter oder Suche prüfen.")}</p>}
    <div className={listSlots ? "min-h-0 min-w-0 flex-1" : "grid min-w-0 gap-3 xl:grid-cols-[12rem_minmax(0,1fr)]"}>
      ...
      </div>, slot, id)) : <aside aria-label={t("Workspace-Spec-Liste")} className="max-h-96 overflow-auto rounded border border-slate-200 bg-white p-2">
      ...
      <div aria-label="Workspace-Board" className={`grid min-w-0 gap-3 ${listSlots ? "h-full min-h-0" : ""}`} style={{ gridTemplateColumns: "repeat(auto-fit, minmax(min(100%, 13rem), 1fr))" }}>
        {stations.map(station => {
          ...
          const card = (entry: WorkspaceSpecEntry) => <button key={entry.key} data-workspace-spec={entry.key} data-selected={selected === entry.key} aria-pressed={selected === entry.key} onClick={() => choose(entry.key)}
            ...
            <span className="mt-2 block text-xs">{entry.spec.tasks_done}/{entry.spec.tasks_total} {t("Aufgaben")}{entry.spec.ready && entry.spec.needs_human ? t(" · Abnahme offen") : ""}{entry.spec.open_question ? t(" · Frage {open_question}", { open_question: entry.spec.open_question }) : ""}{entry.spec.archived ? t(" · Altbestand") : ""}</span>
          </button>;
          ...
          <h3 className={`mb-2 flex flex-wrap items-center justify-between gap-x-2 gap-y-1 text-sm font-semibold ${listSlots ? "tone-ink uppercase" : "text-slate-700"}`}>
            ...
            {station === "Done" && <span className="flex items-center gap-0.5 whitespace-nowrap">
              <select aria-label="Done-Zeitraum" title={t("Nur Specs, die in diesem Zeitraum fertig wurden, liegen offen; ältere sind eingeklappt. Gilt für dieses Workspace-Board.")} value={done.days} onChange={event => done.setDays(Number(event.target.value))} className={DONE_SELECT}>
                {DONE_WINDOW_OPTIONS.map(option => <option key={option.days} value={option.days}>{t(option.label)}</option>)}
              </select>
              <select aria-label="Done-Anzahl" title={t("So viele Karten auf einmal; „Mehr anzeigen“ holt die nächsten. Gilt für dieses Workspace-Board.")} value={done.limit} onChange={event => done.setLimit(Number(event.target.value))} className={DONE_SELECT}>
                {DONE_LIMIT_OPTIONS.map(option => <option key={option.limit} value={option.limit}>{t(option.label)}</option>)}
              </select>
            </span>}
          </h3>
          ...
          {station === "Done"
            ...
            : recent.map(card)}
          {station === "Done" && recent.length === 0 && older.length > 0 && <p className="mb-2 text-[11px] text-slate-500">{t("Nichts in diesem Zeitraum fertig geworden.")}</p>}
    ...
    {detail && !onSelect && <article aria-label={t("Workspace-Spec-Vorschau")} className="min-w-0 rounded-lg border border-slate-200 bg-white p-4">
      ...
      <p className="my-2 text-xs text-slate-500">{t("Nur lesen. Bearbeitung und Abnahme im zugehörigen Projektfenster; dort diese Datei auswählen.")}</p>
