...
interface Snapshot { workspace_id: string; revision: number; captured_at: number; specs: Entry[]; warnings: string[]; partial: boolean }
...
export default function WorkspaceBoardView({ workspaceId, revision }: { workspaceId: string; revision: number }) {
  ...
  const choose = (key: string) => { setSelected(key); setOpenError(null); };
  ...
  return <section aria-label="Workspace-Specs" className="min-w-0 space-y-3">
    ...
    <p role="status" className="text-xs text-slate-500">{loading ? "Snapshot laden…" : `${visible.length} von ${data?.specs.length ?? 0} Specs`}
      {data && ` · Stand ${new Date(data.captured_at).toLocaleTimeString()}`}. Lesende lokale Übersicht, kein Team-Sync. Änderungen nach Aktualisieren sichtbar.</p>
    ...
    <div className="grid min-w-0 gap-3 xl:grid-cols-[12rem_minmax(0,1fr)]">
      <aside aria-label="Workspace-Spec-Liste" className="max-h-96 overflow-auto rounded border border-slate-200 bg-white p-2">
    ...
    {detail && <article aria-label="Workspace-Spec-Vorschau" className="min-w-0 rounded-lg border border-slate-200 bg-white p-4">
