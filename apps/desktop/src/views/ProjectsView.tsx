...
export default function ProjectsView() {
  ...
  return (
    <div className="space-y-8">
      <section aria-label={t("Ordner öffnen")} className="max-w-5xl space-y-3">
        <div>
          <h2 className="text-lg font-semibold text-slate-800">{t("Ordner öffnen")}</h2>
          <p className="mt-1 max-w-3xl text-sm text-slate-500">
            {t("Ein Ordner genügt. Ist er selbst ein Git-Repository oder ein einfaches Projekt, öffnet sich sein Projektfenster. Enthält er mehrere Repositories oder Projekte, wird er als Workspace gespeichert und im gemeinsamen Arbeitsfenster geöffnet.")}
          </p>
        </div>
        <form className="flex flex-wrap items-end gap-2" onSubmit={event => { event.preventDefault(); void openFolder(path); }}>
          <label className="min-w-56 flex-1 text-xs text-slate-500">Ordner
            <input aria-label={t("Ordner")} value={path} onChange={event => setPath(event.target.value)} placeholder={t("~/Projekte/mein-projekt oder ~/Projekte")} spellCheck={false} disabled={busy}
          ...
          <button type="button" className={button} disabled={busy} onClick={async () => {
            try {
              const picked = await open({ directory: true, title: t("Ordner wählen") });
              if (typeof picked === "string") { setPath(picked); void openFolder(picked); }
            } catch (e) { setError(String(e)); }
          }}>{t("Auswählen…")}</button>
          <button type="submit" disabled={busy || !path.trim()} data-tone="blue" className={`${button} tone-surface`}>{busy ? t("Bitte warten…") : t("Öffnen")}</button>
        </form>
        <p className="text-xs text-slate-500">{t("Workspace-Erkennung: 16 Ebenen, ohne Symlinks, Abhängigkeits- und Buildordner. Keine Git- oder Projektdateien werden verändert.")}</p>
        {error && <ErrorBox message={error} />}
        {opened?.kind === "project" && <p role="status" data-tone="green" className="tone-surface rounded p-3 text-xs">{t("Einzelprojekt erkannt. Projektfenster geöffnet:")} <span className="font-mono">{opened.root}</span></p>}
        {opened?.kind === "workspace" && <p role="status" data-tone="green" className="tone-surface rounded p-3 text-xs">{t("Workspace erkannt:")}{" "}{opened.workspace.repositories.length}{" "}{t("Repos/Ordner ·")}{" "}{worktrees(opened.workspace)} Worktrees. Arbeitsfenster geöffnet; Projektgruppen unten anpassen.</p>}
        {(recent.data ?? []).length > 0 && (
          <div>
            <h3 className="mb-1 text-xs font-semibold text-slate-500">{t("Zuletzt geöffnete Projekte")}</h3>
