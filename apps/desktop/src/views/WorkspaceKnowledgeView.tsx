...
export default function WorkspaceKnowledgeView({ workspaceId, root, kind, refresh, onSelect, onManage, onSnapshot }: {
  ...
  return <section aria-label={`Gemeinsame ${labels[kind]}`} className="space-y-2 py-2 text-xs">
    <input aria-label={`${labels[kind]} im Workspace suchen`} placeholder={`${labels[kind]} suchen…`} value={search} onChange={e => setSearch(e.target.value)} className="w-full rounded border p-2" />
    <select aria-label="Wissensquelle filtern" className="w-full rounded border bg-white p-1" value={source} onChange={e => setSource(e.target.value)}><option value="">Alle Quellen</option>{snapshot?.sources.map(source => <option key={source.id} value={source.id}>{source.name}{source.available ? "" : " · nicht verfügbar"}</option>)}</select>
    {kind === "playbooks" && <select aria-label="Workspace-Playbook-Status" className="w-full rounded border bg-white p-1" value={status} onChange={e => setStatus(e.target.value)}><option value="">Alle Status</option>{["active", "draft", "invalid"].map(value => <option key={value} value={value}>{statuses[value]}</option>)}</select>}
    <div className="flex justify-between"><span>{visible.length} von {entries.length}</span><button onClick={() => setReload(value => value + 1)}>Liste aktualisieren</button></div>
    {error && <ErrorBox message={error} />}
    {snapshot?.warnings.map((warning, index) => <p role="status" key={index}>{warning}</p>)}
    {snapshot?.sources.filter(source => source.errors.length || !source.available).map(source => <p role="status" key={source.id}>{source.name}: {source.errors.join(" · ") || "Nicht verfügbar"}</p>)}
    <div aria-label={`${labels[kind]} aller Quellen`} className="space-y-1">
      {visible.map(entry => <button key={entry.key} data-knowledge-key={entry.key} aria-pressed={selected === entry.key} onClick={() => { setSelected(entry.key); onSelect(entry); }} data-tone="violet" className={`block w-full rounded p-2 text-left ${selected === entry.key ? "tone-surface" : "hover:bg-slate-100"}`}>
        <strong className="block">{entry.title}</strong><span className="block break-all">{entry.source} · {entry.host ? `${entry.host} · ` : ""}{statuses[entry.status] ?? entry.status}</span>
      </button>)}
    </div>
    {selected && !current && snapshot && <p role="status">Ausgewählter Eintrag nicht mehr verfügbar. Keine Aktion auf veralteter Quelle ausführen.</p>}
    {current && kind === "mcps" && <div className="space-y-1 rounded border p-2">
      <p>Diese Definition ist im Root-Host noch nicht bestätigt. Die Listenanzeige startet keinen Server.</p>
      <button onClick={() => void copyHandover(mcpConnectionPrompt(current, root)).then(() => setNotice("Anbindungsauftrag kopiert."))}>Anbindungsauftrag kopieren</button>
      <button onClick={() => void deliverToTerminal(mcpConnectionPrompt(current, root)).then(result => setNotice(result.status === "delivered" ? "Im Root-Terminal eingefügt; Enter bestätigt den Auftrag." : "Root-Terminal zuerst starten."))}>Anbindung im Root-Terminal prüfen…</button>
    </div>}
    {notice && <p role="status">{notice}</p>}
    <div className="space-y-1 border-t pt-2">
      <label>Speicher-/Importziel<select aria-label="Wissensziel wählen" className="block w-full rounded border bg-white p-1" value={target} onChange={e => setTarget(e.target.value)}><option value="">Quelle wählen…</option>{snapshot?.sources.filter(source => source.available).map(source => <option key={source.id} value={source.id}>{source.name}</option>)}</select></label>
      <button disabled={!target} onClick={() => onManage(target)} className="rounded border px-2 py-1 disabled:opacity-40">{kind === "skills" ? "Quellen durchsuchen / importieren" : kind === "playbooks" ? "Playbook hier anlegen" : "Im gewählten Ziel verwalten"}</button>
    </div>
  </section>;
}
