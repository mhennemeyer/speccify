...
export default function WorkspaceView({ focus }: { focus?: { id: string; token: number } | null }) {
  ...
  const edit = async (change: Edit) => {
    ...
    try {
      ...
      setNotice(t("Zuordnung gespeichert. Repository-Dateien und laufende Terminals bleiben unverändert."));
    } catch (e) {
      setError(String(e));
  ...
  const openTree = async (tree: Worktree) => {
    ...
    try {
      ...
      setNotice(t("Eigenes Projektfenster geöffnet: {path}. Bestehende Terminals behalten ihr Ziel.", { path: tree.path }));
    } catch (e) { setError(String(e)); }
    finally { setBusy(false); }
  };
  ...
  return <section aria-label="Workspaces" className="max-w-5xl space-y-4">
    <div>
      <h2 className="text-lg font-semibold text-slate-800">{t("Gespeicherte Workspaces")}</h2>
      <p className="mt-1 max-w-3xl text-sm text-slate-500">{t("Ein Arbeitsordner, mehrere Projekte: Repos und Worktrees fachlich gruppieren und gezielt öffnen. Specs gemeinsam überblicken; Dateien, Skills, Git und Terminals bleiben pro Worktree getrennt.")}</p>
    </div>
    ...
    {list.loading && <p role="status" className="text-sm text-slate-500">{t("Workspaces laden…")}</p>}
    {workspace ? <>
      <div className="flex flex-wrap items-center gap-3 rounded-lg border border-slate-200 bg-white p-3">
        <label className="min-w-0 max-w-full text-xs text-slate-500">{t("Gespeicherter Workspace")}
          <select aria-label={t("Gespeicherter Workspace")} disabled={busy} value={workspace.id} onChange={event => select(event.target.value)} className="mt-1 block max-w-full rounded border border-slate-300 bg-white px-2 py-1 text-sm text-slate-800">
            {list.data?.map(entry => <option key={entry.id} value={entry.id}>{entry.name} · {entry.root}</option>)}
          </select>
        </label>
        <button className={button} disabled={busy} onClick={() => void rescan(workspace.root)}>{busy ? t("Bitte warten…") : t("Erneut erkennen")}</button>
        ...
        }}>{t("Workspace öffnen ↗")}</button>
        <span className="text-xs text-slate-500">{t("{repos} Repos/Ordner · {worktrees} Worktrees", { repos: workspace.repositories.length, worktrees: workspace.repositories.reduce((sum, repo) => sum + repo.worktrees.length, 0) })}</span>
        <p className="w-full break-all font-mono text-xs text-slate-500">{workspace.root}</p>
      </div>
      <nav aria-label={t("Workspace-Ansicht")} className="flex gap-2">
        ...
        <button data-tone="violet" className={`${button} ${mode === "specs" ? "tone-surface" : ""}`} aria-pressed={mode === "specs"} onClick={() => setMode("specs")}>{t("Alle Specs")}</button>
      </nav>
      {(workspace.partial || workspace.warnings.length > 0) && <aside role="status" data-tone="amber" className="tone-surface rounded border p-3 text-xs">
        <strong>{workspace.partial ? t("Suche begrenzt") : t("Hinweise zur Erkennung")}</strong>
        <ul className="mt-1 list-inside list-disc">{workspace.warnings.map((warning, index) => <li key={index}>{warning}</li>)}</ul>
        <p className="mt-1">{t("Verfügbare Projekte können geöffnet werden. Bekannte Zuordnungen bleiben erhalten; nicht durchsuchte Unterordner bei Bedarf separat öffnen.")}</p>
      </aside>}
      ...
      <form onSubmit={event => { event.preventDefault(); void edit({ kind: "group", repository_ids: selected, name: groupName }); }} data-tone="violet" className="tone-surface flex flex-wrap items-center gap-2 rounded-lg border p-3">
        <span className="text-xs">{selected.length}{" "}{t("ausgewählt")}</span>
        <input aria-label={t("Name der Projektgruppe")} value={groupName} onChange={event => setGroupName(event.target.value)} disabled={busy} maxLength={100} placeholder={t("Gemeinsames Projekt, z. B. Kundenportal")} className="min-w-64 flex-1 rounded border border-slate-300 bg-white px-2 py-1.5 text-sm" />
        <button className={`${button} tone-surface`} disabled={busy || selected.length < 2 || !groupName.trim()}>{t("Als Projekt gruppieren")}</button>
      </form>
      {workspace.projects.filter(project => project.repository_ids.length > 0).map(project => <article key={project.id} data-project-id={project.id} className="rounded-lg border border-slate-200 bg-white p-4">
        <header className="mb-3 flex flex-wrap items-center justify-between gap-2">
          {renaming === project.id ? <form className="flex gap-2" onSubmit={event => { event.preventDefault(); void edit({ kind: "rename", project_id: project.id, name: renameValue }); }}>
            ...
            <button className={button} disabled={busy || !renameValue.trim()}>{t("Speichern")}</button>
            <button type="button" className={button} disabled={busy} onClick={() => setRenaming(null)}>{t("Abbrechen")}</button>
          </form> : <>
            ...
            <button className={button} disabled={busy} onClick={() => { setRenaming(project.id); setRenameValue(project.name); }}>{t("Umbenennen")}</button>
          </>}
        </header>
          ...
          return <div key={repo.id} data-repository-id={repo.id} className="rounded border border-slate-200 bg-slate-50 p-3">
            <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
              <label className="flex items-center gap-2 text-sm font-medium text-slate-700"><input type="checkbox" disabled={busy} checked={selected.includes(repo.id)} onChange={event => setSelected(previous => event.target.checked ? [...previous, repo.id] : previous.filter(id => id !== repo.id))} />{repo.name}
                <span className="text-xs font-normal text-slate-500">{repo.common_dir ? "Git" : t("Ordner ohne Git")}{repo.worktrees.length > 1 ? t(" · {worktrees} Worktrees, ein Repo", { worktrees: repo.worktrees.length }) : ""}</span>
              </label>
              {project.id !== repo.default_project_id && <button className={button} disabled={busy} onClick={() => void edit({ kind: "ungroup", repository_id: repo.id })}>{t("Aus Gruppe lösen")}</button>}
            </div>
            {repo.worktrees.map(tree => <div key={tree.id} className="flex flex-wrap items-center justify-between gap-2 border-t border-slate-200 py-2">
              <div className="min-w-40 max-w-full flex-1"><p className="break-all font-mono text-xs text-slate-600" title={tree.path}>{tree.relative_path || "."}</p>
                <p className="mt-1 break-all text-[11px] text-slate-500">{tree.markers.join(" · ") || t("Keine Projektanweisungen erkannt")}{!tree.available && t(" · Nicht verfügbar")}</p>
              </div>
              <button className={button} disabled={busy || !tree.available} title={tree.path} aria-label={t("Worktree öffnen: {path}", { path: tree.path })} onClick={() => void openTree(tree)}>{t("In eigenem Fenster öffnen ↗")}</button>
            </div>)}
          </div>;
        })}</div>
      </article>)}
      </>}
    </> : !list.loading && !list.error && <p className="rounded border border-dashed border-slate-300 p-6 text-sm text-slate-500">{t("Noch kein Workspace gespeichert. Öffne oben einen Ordner, der mehrere Repositories oder Projekte enthält.")}</p>}
  </section>;
}
