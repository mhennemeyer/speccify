...
function Field({ field, value, edit, onChange }: {
  ...
  return <div className="rounded border border-slate-200 bg-white p-3">
    <div className="mb-1 flex items-start justify-between gap-2">
      ...
      <button className="shrink-0 text-xs text-blue-700" onClick={() => onChange("null")}>{t("Nicht setzen")}</button>
    </div>
    {useSelect ? <select id={`setting-${key}`} value={text} className={inputClass} onChange={e => onChange(e.target.value)}>
      <option value="null">{t("Nicht gesetzt · Host-Standard / andere Ebene")}</option>
      ...
      placeholder={t("Nicht gesetzt")} onChange={e => onChange(JSON.stringify(e.target.value))} />
      : numeric ? <input id={`setting-${key}`} type="number" className={inputClass}
        ...
        value={typeof parsed === "number" ? parsed : ""} placeholder={t("Nicht gesetzt")}
    ...
    {options && field.type === "json" && <button className="mt-1 text-xs text-blue-700" onClick={() => onChange("{}")}>{t("Komplexen JSON-Wert bearbeiten")}</button>}
    <p className="mt-2 whitespace-pre-wrap text-xs text-slate-500">{t(field.help)}</p>
    {!simple && !numeric && !useSelect && <p className="mt-1 text-xs text-slate-500">{t("JSON-Wert; null entfernt die Einstellung aus dieser Datei.")}</p>}
...
function ConfigEditor({ file, onDirty }: { file: AgentConfigFile; onDirty: (dirty: boolean) => void }) {
  ...
  const [group, setGroup] = useState(t("Berechtigungen und Sandbox"));
  ...
  async function materialize() {
    const changes = Object.entries(edits).map(([key, text]) => {
      ...
      catch { throw new Error(t("{key}: ungültiger JSON-Wert", { key })); }
    });
  ...
  async function save() {
    ...
    try {
      ...
      setStatus(t("Gespeichert. Beim nächsten Agent-Start wirksam. Laufende Sitzung bei Bedarf neu starten."));
    } catch (e) { setError(String(e)); } finally { setBusy(false); }
  }
  ...
  const shown = fields.filter(field => search
    ? `${field.path.join(".")} ${t(field.help)}`.toLowerCase().includes(search.toLowerCase()) : field.group === group);
  return <div className="flex min-h-0 flex-1 flex-col gap-3" data-update-dirty={dirty}>
    ...
    <p className="text-xs text-slate-600">{t("Globale Einstellungen für diesen Benutzer und alle Projekte. Projektdateien, aktive Profile, Startparameter und verwaltete Regeln können Werte übersteuern. Der Agent liest die Datei beim Start; dies ist keine Anzeige der effektiven Sitzungseinstellungen.")}</p>
    <div className="flex flex-wrap gap-2">
      <button className={buttonClass} disabled={!loaded || busy || !dirty} onClick={() => void save()}>{t("Speichern")}</button>
      <button className={buttonClass} disabled={!dirty || busy} onClick={() => void reset()}>{t("Änderungen zurücknehmen")}</button>
      {structured && <button className={buttonClass} disabled={!loaded || busy} onClick={() => void changeView()}>{source ? t("Strukturierte Ansicht") : t("Quelltext")}</button>}
      {structured && <button className={buttonClass} onClick={() => void openUrl(settingsReferences[file.id])}>{t("Referenz öffnen")}</button>}
    </div>
    {error && <ErrorBox message={error} />}
    {status && !dirty && <p role="status" className="text-xs text-emerald-700">{status}</p>}
    {!loaded && <p className="text-xs">{t("Datei wird gelesen…")}</p>}
    {loaded && (source || !structured) ? <textarea aria-label={t("Agent-Konfiguration Quelltext")} spellCheck={false}
      ...
      : loaded && <fieldset disabled={busy} className="flex min-h-0 flex-1 flex-col gap-3">
        <details className="rounded border border-blue-200 bg-blue-50 p-3 text-xs">
          <summary className="cursor-pointer font-semibold">{t("Empfohlener Ausgangspunkt: Selbstständig arbeiten")}</summary>
          <p className="my-2">{file.host === "codex"
            ? t("on-request + auto_review + workspace-write: Automatische Prüfung geeigneter Freigaben, Änderungen im Arbeitsordner, Netzwerk bleibt separat geregelt. Terminalmeldungen werden aktiviert. Aktuelle Codex-Version nötig. Unter Windows zusätzlich windows.sandbox prüfen und Codex-Sandbox einmalig einrichten.")
            : t("permissions.defaultMode = auto: Automatische Befehlsprüfung. Konto, Modell und Admin-Regeln müssen Auto erlauben; explizite ask-Regeln können weiter fragen. acceptEdits ist eine Alternative, reduziert aber hauptsächlich Rückfragen bei Dateiänderungen.")}</p>
          <p className="my-2">{t("Das Profil verändert nur die unten aufgeführten Werte. Bestehende Regeln und übrige Einstellungen bleiben erhalten. Es wird erst mit Speichern übernommen.")}</p>
          ...
          <button className={buttonClass} onClick={() => {
            ...
            setStatus("");
          }}>{t("Profil in Entwurf übernehmen")}</button>
        </details>
        <input aria-label={t("Agent-Einstellungen suchen")} placeholder={t("Einstellung oder Hilfetext suchen…")} className={inputClass}
        ...
        <div className="min-h-0 flex-1 space-y-2 overflow-y-auto">
          ...
          {!shown.length && <p className="text-sm text-slate-500">{t("Keine passende Einstellung gefunden.")}</p>}
        </div>
      </fieldset>}
  </div>;
}
...
export default function AgentsView() {
  ...
  return <LoadingBoundary loading={list.loading} error={list.error} label={t("Agent-Configs suchen…")}>
    <div className="flex h-full min-h-0 flex-col gap-3">
      ...
      }}>{t("Datei neu laden")}</button></nav>
      {switchTo && <div role="alert" className="rounded border border-amber-300 p-3 text-sm">{t("Ungespeicherte Änderungen verwerfen?")}
        <button className={`${buttonClass} ml-2`} onClick={() => { setSelected(switchTo); setSwitchTo(null); setDirty(false); setRevision(value => value + 1); }}>{t("Verwerfen und wechseln")}</button>
        <button className={`${buttonClass} ml-2`} onClick={() => setSwitchTo(null)}>{t("Weiter bearbeiten")}</button>
      </div>}
