...
export function StartupDetails({ report }: { report: AgentStartupReport }) {
  ...
  return (
    <div className="space-y-1 break-words text-xs">
      ...
      <p>Agent: <code>{report.host_path ?? (report.host === "shell" ? t("nur Shell") : t("nicht geprüft"))}</code>{report.host_version ? ` · ${report.host_version}` : ""}</p>
      <p>Speccify ({source}): <code>{report.effective_cli ?? t("nicht gefunden")}</code></p>
      {report.shell_cli && report.shell_cli !== report.effective_cli ? <p>Shell-Installation: <code>{report.shell_cli}</code></p> : null}
      {report.effective_cli && report.missing_commands.length === 0 ? <p>{t("expand, export, link und tool verfügbar.")}</p> : null}
  ...
  return (
    <div className="mt-3 space-y-2 text-xs">
      <button type="button" disabled={busy} onClick={() => void check()} className="rounded border border-slate-400 px-2 py-1 disabled:opacity-50">
        {busy ? t("Startumgebung wird geprüft…") : t("Startumgebung prüfen")}
      </button>
