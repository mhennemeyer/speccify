...
def test_pull_renders_files_and_updates_lockfile(tmp_path: Path) -> None:
    ...
    out = project / "out"
    result = runner.invoke(app, ["pull", "--project", str(project), "--out", str(out)])
    ...
    assert button_entry.generated_files_sha256[0].path == "org/button.md"
...
def test_pull_is_deterministic(tmp_path: Path) -> None:
    ...
    out = project / "out"
    runner.invoke(app, ["pull", "--project", str(project), "--out", str(out)])
    first_lock = (project / "speccify.lock").read_bytes()
    first_file = (out / "org" / "button.md").read_bytes()

    runner.invoke(app, ["pull", "--project", str(project), "--out", str(out)])
    second_lock = (project / "speccify.lock").read_bytes()
    second_file = (out / "org" / "button.md").read_bytes()
...
def test_pull_fails_without_lockfile(tmp_path: Path) -> None:
    ...
    result = runner.invoke(app, ["pull", "--project", str(project), "--out", str(project / "out")])
